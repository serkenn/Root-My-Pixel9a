...
Pixel 9a を実際に動かすには、この環境では用意できないバイナリが 2 つ必要です。

1. **`app/src/main/assets/exploits/tegu-CP2A.260705.006.so`**
   `./build-all.sh` でビルドできます（`TARGETS` に追加済み）。Android NDK が必要です。

2. **`app/src/main/assets/ksud/ksud-android14-6.1`**
   現在同梱されているのは `ksud-android15-6.6` のみです。ReSukiSU / KernelSU のリリースから、`android14-6.1` KMI 向けの `ksud` を入手して配置してください。これが無いと、エクスプロイト自体が成功してもペイロード展開の段階で失敗します。

### 検証状況

- ✅ 全オフセットが実イメージ由来（推測なし）
- ✅ 4 機種すべてのターゲットヘッダが静的検証（コンパイル時アサーション）を通過
- ✅ `slide.c` の変更が既存 3 機種で完全に等価であることを確認
- ❌ **実機での動作確認は未実施**

最後の 1 点が本質的です。すべての値に出典があることと、動作することは別です。実機で試す場合は、上の警告を読み直してください。
...
| `Failed to extract ksud` | その KMI 用の `ksud` が同梱されていません（Pixel 9a は現状これに該当、§6 参照） |
